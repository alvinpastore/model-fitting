...
SAVE_FIG = 1;               % flag to handle figure saving on disk (0 does NOT save figures, 1 saves figures)
...
if FIGURES(1)
    ...
    % sort players according to performances 
    btr_ranked_performances = sortrows(stats(better_than_random(:,1)+1,:),8);

    % on the x-axis there is profit
    btr_x = btr_ranked_performances(:,8);

    % on the y-axis there is MLE (best and random)
    y_best = btr_ranked_performances(:,5);
    y_random = btr_ranked_performances(:,6);
    ...
    labels_text = text(btr_x + dx, y_best+dy ,btr_labels,'FontSize',ID_TEXT_SIZE);
...
if FIGURES(2)
    ...
    labels_text = text(btr_x + dx, btr_y_alpha+dy ,btr_labels,'FontSize',ID_TEXT_SIZE);
...
if FIGURES(3)
    ...
    labels_text = text(btr_x + dx, btr_y_gamma+dy ,btr_labels,'FontSize',ID_TEXT_SIZE);
...
if FIGURES(4)
    ...
    labels_text = text(btr_x + dx, btr_y_beta+dy ,btr_labels,'FontSize',ID_TEXT_SIZE);
